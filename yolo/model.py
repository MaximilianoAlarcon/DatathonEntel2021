...
class Model:
    ...
    def excecute_sdf(self):
            ...
            if len(indexes) > 0:
                for i in indexes.flatten():
                    ...
                    if class_ids[i] == 1:
                        ...
                        cv2.imwrite('./fechas/{}'.format(image_name), cropped)
        ...
        sdf_output.to_csv('./salida/predicciones.csv', index=False)
    ...
    def execute_docr(self, partial):
        ...
        model_output = pd.read_csv('./salida/predicciones.csv')
        ...
        for i in images:
            ...
            # Lectura de imagenes
            img = cv2.imread('./fechas/{}'.format(image_name))
            height, width, _ = img.shape
        ...
        model_output.to_csv('./salida/predicciones.csv', index=False)